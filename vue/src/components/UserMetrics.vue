<template>
  <div class="metrics-container">
    ...
    <table class="metrics-table">
      ...
      <tbody>
        <tr v-for="(metric, index) in groupedMetrics" :key="index" @click="selectMetric(metric)" class="metric-row"
    ...
    <div v-show="showWorkout">
      ...
      <table class="workout-details-table">
        ...
        <tbody>
          <tr v-for="(workout, index) in workouts" :key="workout.exerciseName" :class="{ 'odd-row': index % 2 !== 0 }">
      ...
      this.caloriesChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: 'Calories Burned',
              data: dates.map((date, index) => ({ x: date, y: calories[index] })), 
              borderColor: 'rgba(75, 192, 192, 1)',
              backgroundColor: 'rgba(75, 192, 192, 0.2)',
              fill: true,
              tension: 0.1,
